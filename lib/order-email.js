...
async function sendNotificationEmail({ subject, html, idempotencyKey }) {
    ...
    const result = await resend.emails.send(
        { from: sender, to: recipient, subject, html },
        idempotencyKey ? { idempotencyKey } : undefined
    );

    if (result.error) {
        const error = new Error(result.error.message || "Email failed");
...
async function sendDesignInquiry(inquiry) {
    const items = (inquiry.items || []).map(item =>
        `<li>${escapeHtml(item.name)} × ${escapeHtml(item.quantity)}${item.selections?.[0] ? ` (${escapeHtml(item.selections[0])})` : ""}</li>`
    );
    return sendNotificationEmail({
        subject: `New Terra Haven Design Inquiry – ${inquiry.name}`,
        html: `<h1>New Terra Haven Design Inquiry</h1><p><strong>Name:</strong> ${escapeHtml(inquiry.name)}<br><strong>Email:</strong> ${escapeHtml(inquiry.email)}<br><strong>Phone:</strong> ${escapeHtml(inquiry.phone)}<br><strong>Bundle:</strong> ${escapeHtml(inquiry.bundle)}<br><strong>Bundle Total:</strong> ₹${escapeHtml(inquiry.bundleTotal)}</p><h2>Products</h2><ul>${items.join("")}</ul><p><strong>Customization:</strong> ${escapeHtml(inquiry.customizationDetails)}<br><strong>Gift Message:</strong> ${escapeHtml(inquiry.giftMessage)}<br><strong>Additional Notes:</strong> ${escapeHtml(inquiry.additionalNotes)}</p>`
    });
}
...
async function sendNewsletterSignup(email) {
    ...
    return sendNotificationEmail({
        subject: "New Terra Haven Newsletter Signup",
        html: `<h1>New Terra Haven Newsletter Signup</h1><p><strong>Subscriber:</strong> ${escapeHtml(email)}<br><strong>Date/Time:</strong> ${timestamp}<br><strong>Source:</strong> Terra Haven website</p>`,
        idempotencyKey: `terra-newsletter-${email.toLowerCase()}`
    });
}
